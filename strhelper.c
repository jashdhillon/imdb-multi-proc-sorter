...
int find(char** list, int list_len, char* target) {
	...
	for(i = 0; i < list_len; i++) {
		...
		trim(list[i], &str_start, &str_end);

		char* buffer = malloc((str_end - str_start + 2) * sizeof(char));
		
		for(j = 0; j < (str_end - str_start + 1); j++) {
			buffer[j] = list[i][str_start + j];
		}
		buffer[j] = '\0';
		
		if(is_string_equal(buffer, target)) {
		    free(buffer);
		    return i;
		}
		free(buffer);
